...
function updateAuthState() {
  ...
  if (welcomeUser) {
    welcomeUser.textContent = authed ? `Signed in as ${firebaseAuth.currentUser.email}` : '';
  }
...
if (authForm) {
  authForm.addEventListener('submit', async (event) => {
    ...
    if (!email || !password) {
      if (authFeedback) authFeedback.textContent = 'Enter both email and password.';
      return;
    }

    if (!firebaseInitialized || !firebaseAuth) {
      if (authFeedback) authFeedback.textContent = 'Authentication is not initialized yet.';
      return;
    }

    try {
      await firebaseAuth.signInWithEmailAndPassword(email, password);
      if (authFeedback) authFeedback.textContent = 'Login successful. Redirecting...';
      updateAuthState();
    } catch (error) {
      console.error('Firebase sign-in error:', error);
      if (authFeedback) authFeedback.textContent = 'Login failed. Check your email and password.';
    }
  });
}
...
inventoryItems = loadInventory();
updateAuthState();
>>>>>>> 2c7c2f1443cf8ea746ec862d50395a3809adc846
